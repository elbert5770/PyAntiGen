...
def create_project():
    ...
    # Copy the package's .agents/skills into project top-level .agents (so agents run in project context)
    ...
    # No Engine is copied: since 2.0 it is pyantigen.engine, imported from
    # the installed package, so every project runs the same version of it.
    ...
    print("neither the model generator nor the Engine is copied here. Import them")
    print("directly: 'from pyantigen.generate.pyantigen import PyAntiGen',")
    print("'from pyantigen.engine.Model_optimize import setup_optimization_from_groups'.")

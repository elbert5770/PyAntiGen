...
class EvalSpec:
    ...
    # pyantigen.engine.Noise_floor.export_cache() snapshot, taken in the parent
    # AFTER its own calibration (see pyantigen.engine.Optimize.run_optimization_from_groups,
    ...
    # vectors at the model that are mostly nonsense; see pyantigen.engine.Simulate's
...
def _init_worker(spec_blob):
    ...
    from pyantigen.engine.Optimize import OptRoadRunnerProxy
    from pyantigen.engine.Noise_floor import seed_cache

    spec = _serializer.loads(spec_blob)
    # Before any task runs: this worker's own pyantigen.engine.Noise_floor
    ...
    from pyantigen.engine.Simulate import set_search_mode
    ...
    for sim_name, replicate in spec.replicates.items():
        ...
        if getattr(spec, "preequil_cache", False) and not spec.events_dynamic:
            from pyantigen.engine.Preequil_cache import PreequilCache
...
def _worker_nll(x, frozen_sigmas=None):
    ...
    from pyantigen.engine.Optimize import evaluate_nll_fixed
...
def _profile_task(job):
    ...
    **A point survives being killed.** A point can take hours, and on a
    ...
    :mod:`pyantigen.engine.Nelder_mead`, whose entire state is a small dict, and rewrites
    ...
    * ``nm_simplex`` -- for Nelder-Mead, the whole simplex, so a job started
    ...
    try:
        ...
        # pyantigen.engine.Optimize._freeze_floor) -- stamped onto the job by
...
class ParallelEvaluator:
    ...
    def profile_batch(self, jobs, on_result=None, label=None,
        ...
        optimizer state (see ``pyantigen.engine.Profile_checkpoint.point_state_path``).
        ...
        obs_label): sigma_used}`` lookup (see ``pyantigen.engine.Optimize.
        ...
        ``pyantigen.engine.Optimize._freeze_floor``. Every profile pass submitted
        ...
        * A point is admitted against the clock as it is about to *start*, not
        ...
        :class:`~pyantigen.engine.Deadline.DeadlineReached` is raised naming how many never
...
def build_eval_spec(
    ...
    from pyantigen.engine.Noise_floor import export_cache

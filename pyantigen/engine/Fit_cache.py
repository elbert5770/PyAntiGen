...
* ``partial`` -- a fit that has not returned. Always the best point seen so far,
  ...
  complete state (pyantigen.engine.Nelder_mead), written every few seconds by the fit
...
from pyantigen.engine.Profile_checkpoint import _safe_name, sweep_stale_temp_files
...
class FitCache:
    ...
    def _de_state(self, block):
        ...
        if not isinstance(block, dict):
            return None
        from pyantigen.engine.Differential_evolution import state_from_json
    ...
    def _nm_state(self, block):
        ...
        if not isinstance(block, dict):
            return None
        from pyantigen.engine.Nelder_mead import state_from_json
    ...
    def load_partial(self):
        ...
        :func:`pyantigen.engine.Nelder_mead.state_from_json` returns it, when one was saved
    ...
    def save_partial(self, x_lin, fun, n_evals=None, force=False, nm_state=None,
        ...
        *nm_state*, when given, is the optimizer's state as
        :func:`pyantigen.engine.Nelder_mead.state_to_json` returns it -- plain lists and
        ...
        *de_state* is the differential-evolution search's state
        (:func:`pyantigen.engine.Differential_evolution.state_to_json`). A fit that runs
        ...
        *cal_x_lin* is the parameter vector (linear units) at which the run's noise
        ...
        every process: pyantigen.engine.Noise_floor calibrates a floored observable on the

...
see a model prediction or model parameter. See ``pyantigen.engine.Optimize.
...
def clear_cache():
    ...
    at a converged optimum -- see ``pyantigen.engine.Optimize.run_optimization_from_groups``.
...
def export_cache():
    ...
    worker (``pyantigen.engine.Evaluator``, ``mp_context="spawn"``) re-imports this
    ...
    through as a field on ``pyantigen.engine.Evaluator.EvalSpec`` the way

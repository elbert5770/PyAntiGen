...
def run_all(r, exp_num, experiment, df_dict, set_parameters=None, parameters=None,
    ...
    which ``pyantigen.engine.Preequil_cache.verify_invariance`` establishes at startup and
...
def _sigma_floor_for(obs_cfg, t_data, y_data, y_sim_ref=None):
    ...
    ``pyantigen.engine.Noise_floor`` so the LOOCV span search itself only ever runs
    once per observable.
    ...
    function and the ``pyantigen.engine.Noise_floor`` module docstring for why
    ...
    *blocks* maps a key to ``[weighted_sse, n_eff, known_sigma_or_None,
    ...
    for v in blocks.values():
        ...
        if ks is not None:
            ...
            # pyantigen.engine.Optimize._freeze_floor) with a sigma pinned from the
...
def _make_anchor_cache(paths, tag, param_names, x_opt, groups, scales,
    ...
    """
    try:
        from pyantigen.engine.Profile_checkpoint import spec_fingerprint, default_run_id
...
def _minimize_nuisance(fun, x0, args, method, bounds, optimizer_kwargs=None,
    ...
    *extra_options* is merged last and is how a resumed point carries its
    ...
    profile worker now runs Nelder-Mead through pyantigen.engine.Nelder_mead, which
...
def _restore_calibration(objective, resume, scales):
    ...
    pyantigen.engine.Noise_floor calibrates each floored observable on the first call it
...
def _run_fit_nelder_mead(objective, x_start, bounds, opt_kw, scales, fit_cache,
    ...
    :mod:`pyantigen.engine.Nelder_mead` is scipy's optimizer with its state in a dict that
    ...
    :class:`~pyantigen.engine.Deadline.DeadlineReached`; being killed instead is the same
    ...
    *x_start*, which the caller has already set to the best point on record.
    ...
    from pyantigen.engine.Fit_cache import NM_STATE_SAVE_INTERVAL_S
...
# names mostly mean something else to the search in pyantigen.engine.Differential_evolution
...
_DE_DEFAULTS = {
    ...
    # 10,000 leaves 4.5x headroom. See pyantigen.engine.Simulate "Search mode". None turns
...
def _run_fit_differential_evolution(objective, x0, bounds, opt_kw, scales,
    ...
    The search (:mod:`pyantigen.engine.Differential_evolution`) evaluates a generation at a
    ...
    *build_pool(search_max_steps)* returns a ParallelEvaluator whose workers are in
    ...
    """
    import pyantigen.engine.Simulate as _sim
    from pyantigen.engine.Deadline import DeadlineReached
...
def _enable_preequil_cache(models, active_replicates, param_names, x0_lin,
    ...
    from pyantigen.engine.Preequil_cache import PreequilCache, split_preequil_block, verify_invariance
...
def _try_build_evaluator(model_text, paths, models, active_replicates, param_names,
    ...
    """
    try:
        from pyantigen.engine.Evaluator import (
...
def _run_parallel_profile_with_checkpoint(
    ...
    from pyantigen.engine.Deadline import RunBudget, resolve_deadline
    from pyantigen.engine.Identifiability import screen_or_raise, screen_summary
...
def _run_fast_profile_with_checkpoint(
    ...
    """
    from pyantigen.engine.Fast_profile import (
        DEFAULT_NEAR_ZERO_FRAC, DEFAULT_ROUND_EVALS, DEFAULT_ROUNDS,
        run_fast_profile, save_report,
    )
    from pyantigen.engine.Identifiability import MIN_REACH_DECADES, SPAN_DECADES
    from pyantigen.engine.Profile_checkpoint import (
        ProfileCheckpoint, spec_fingerprint, default_run_id, solver_fingerprint,
    )
    from pyantigen.engine.Deadline import RunBudget, resolve_deadline
...
def run_optimization(
    ...
    if res.success:
        ...
        if slice_analysis or profile_likelihood_analysis or fast_profile_likelihood_analysis:
            ...
            if sobol_analysis:
                from pyantigen.engine.Sensitivity_analysis import run_sobol_analysis
...
def run_optimization_from_groups(
    ...
    reuse_fit : under fit_mode="optimize", look for a finished fit of exactly
        ...
        always fits afresh. See pyantigen.engine.Fit_cache.
    ...
    if optimization_spec is not None:
        ...
        # spec's fields get threaded into (pyantigen.engine.Evaluator's EvalSpec,
        ...
        for sim_name in unique_sim_names:
            ...
            # pyantigen.engine.Event_times. A solver setting that does not ask for them is
            # unaffected.
        ...
        if fit_mode == "optimize" and reuse_fit:
            from pyantigen.engine.Fit_cache import FitCache, fit_fingerprint, data_fingerprint
            from pyantigen.engine.Profile_checkpoint import solver_fingerprint
        ...
        if res is not None:
            ...
            # Not scipy's: the search in pyantigen.engine.Differential_evolution, which
        ...
        # with. See pyantigen.engine.Noise_floor.clear_cache.
        #
        ...
        try:
            ...
            if slice_analysis or profile_likelihood_analysis or fast_profile_likelihood_analysis:
                ...
                if fast_profile_likelihood_analysis:
                    ...
                    # profile reads. See pyantigen.engine.Fast_profile.
...
def predict_concentrations(params, observable_data, model, param_names, observable_defs=None):
    ...
    if isinstance(observable_data, dict):
        ...
        try:
            ...
            from pyantigen.engine.Simulate import safe_simulate
            block = {
                "start": 0.0,
                "end": 1.0,
                "n_points": 2,

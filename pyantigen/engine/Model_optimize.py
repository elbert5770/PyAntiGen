...
from pyantigen.generate.TelluriumGen import TelluriumGen  # used by _run_steady_state
...
from pyantigen.engine.Identifiability import UnidentifiableParameters
from pyantigen.engine.Results import log_optimization_results
from pyantigen.engine.Petab_export import export_petab
from pyantigen.engine.Model_simulate import _project
...
# point lands in the checkpoint the full profile reads. See pyantigen.engine.Fast_profile.
...
def _run_fast_profile_report(opt, settings, optimization_spec=None):
    ...
    from pyantigen.engine.Fast_profile import fast_profile_summary, print_fast_profile_report
...
def setup_optimization(settings, optimization_settings, experiment_dict):
    ...
    if optimization_settings.get("sobol_analysis") and opt["stats"].get("sobol"):
        from pyantigen.engine.Sensitivity_analysis import save_sobol_plot
...
def setup_optimization_from_groups(settings, optimization_settings, EXPERIMENT_dict):
    ...
    # An Optimization spec is recognised by shape, not by class: the class is
    # defined in each project's Optimizer_settings, which the Engine must not
    # import.
    if hasattr(optimization_settings, "groups") and hasattr(optimization_settings, "param_names"):

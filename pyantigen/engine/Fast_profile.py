...
The point
---------
The slice screen (:mod:`pyantigen.engine.Identifiability`) has already walked each
...
from pyantigen.engine.Evaluator import FAILURE_VALUE
from pyantigen.engine.Identifiability import (
    MIN_REACH_DECADES,
    SPAN_DECADES,
    THRESHOLD,
...
def run_fast_profile(batch, nll_batch, res_x, nll_at_optimum, param_names,
    ...
    *nll_at_optimum* anchors the screen, which goes through *nll_batch* and so
    is never sigma-frozen (see ``pyantigen.engine.Optimize._freeze_floor``).
    ...
    *batch*, which the caller submits with each floored block's sigma pinned
    ...
    not the raw floor -- see ``pyantigen.engine.Optimize._freeze_floor``). Reusing the
    ...
    """
    from pyantigen.engine.Optimize import _cold_simplex, _param_bounds

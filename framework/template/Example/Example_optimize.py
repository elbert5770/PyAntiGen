...
from Modules.AntimonyGen import AntimonyGen
...
def main():
    ...
    # Define parameters to optimize (names must match Antimony model parameter ids).
    # Replace with your model's parameter names and bounds.
    param_names = ["k_A_to_B"]  # e.g. ["k1", "k2"]
    x0 = [0.5]          # e.g. [0.1, 0.05]
    bounds = [(0.0, 1.0)]    # e.g. [(1e-6, 10), (1e-6, 10)]
    ...
    opt = run_optimization(
        ...
        x0=x0,
        bounds=bounds,
        loss_config={"observable": "[B_Comp1]", "data_column": "B", "time_column": "time"},
        method="L-BFGS-B",
    )

...
| Bloat | Factored: each factor level's attributes written once; simulation *generators* stored, never their product |
| Reasoning | Per-project `remarks.json`, time-stamped, append-only, linked by id; rendered to `remarks.html#<id>` |
| Pairing | Every simulation names its subject; contrasts pair within subject by default |
| Reading | Authored by concept (factors, subjects, protocols, assays); the per-simulation view is generated by `describe`, never hand-written |
| Study vs fit | A Study is a paper: its experiments and every dataset it reports. An Optimization is a fit: the parameters it estimates and the datasets it uses, from one or more studies |
| Provenance | A Study carries its `doi` -- one study, one publication; each dataset carries `source`, the figure or table it came from |
...
Protocol   events / solver / observed functions, stored as "module:qualname";
           optionally a data loader (inputs for the events, prepared
           tables), update hooks run after resolve(), and settings every
           simulation under it shares
Simulation subject x protocol x remaining factor levels (x period): one thing
           the Engine integrates. simulate() adds one, simulate_all() every
           combination. An "arm" is only a selection of simulations.
Assay      Measured observables: model Obs + DataSource + Noise (+ only-filter),
           and on= which simulations it scores: all, a select() filter, or
           a Contrast. Obs is an expression or a sum of columns as % of its
           own baseline
Contrast   Contrast.ratio_pct / Contrast.diff(numerator, denominator):
           pairing = "subject" (default) | "between"
Rule       applied last: "k_oligo1 = 0 when amyloid_positive is False"

Optimization  a fit: Params, optimizer settings, and an ordered list of uses
Use        study + assays (whole, or "assay.observable") + optional on=
           filter; or simulate_only() for plotted, unscored simulations
Param      fitted: global, or by=<factor> (one value per level across the
           studies used). Values a design fixes per level live on the
           factor level, in the Study.
```

`resolve(study, simulation, theta, params)` gives every parameter value the
design and the fit set for a simulation, in the order level params, fitted
theta, rules. It reads nothing from a RoadRunner instance.

An assay's `on=` says where that dataset EXISTS; an Optimization chooses what
to score. The same paper can serve several fits, and one fit can pull from
several papers. Alternative readings of one dataset (a fragment treated as a
proteolysis product or as a gamma-secretase product, say) are two assays of
the same study; each fit picks the reading it adopts. Optimizations are
recorded in their own JSON, naming each study used with its fingerprint, so a
record cannot be replayed against a design that has since changed.

`describe(study, estimation)` prints the per-simulation view: for each
simulation its subject, protocol, levels and what they imply, covariates,
protocol settings, functions, fixed/fitted parameters and rules, and every
assay scoring it (model, data table, noise, contrast partner), plus which
simulations nothing scores and which serve as a contrast's control. Loss
elements are emitted in that same order -- simulation by simulation, assays
in declaration order -- which is also the order the Engine sums blocks in.
...
Froehlich-sized full cross (290 x 33 = 9,570 simulations) and writes it as
537 KB of readable JSON, 34 times smaller. Three mechanisms:

1. factor attributes are stored on the level, once;
2. `Study.simulate_all` is stored as one record, not 9,570;
...
Because every simulation carries its subject, `Contrast(pairing="subject")`
...
effect per subject x period) has a place to live: the simulation.
...
| 3 | `Param` + `resolve()`; arm de-duplication | `resolve()` and by-level Params done; `validate` warns on duplicate simulations; Engine-side de-duplication not started |

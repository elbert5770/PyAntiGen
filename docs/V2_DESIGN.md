...
| Reading | Authored by concept (factors, subjects, protocols, assays, params); the per-simulation view is generated by `describe`, never hand-written |
...
`resolve(study, simulation, theta)` gives every parameter value the design
sets for a simulation, in the order level params, fixed Params, fitted theta,
